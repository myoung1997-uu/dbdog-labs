...
//   CLAUDE_PLUGIN_OPTION_CASE_FEED_URL   plugin.json 的 userConfig 在启用时问到的值
...
export function platformUrl() {
  const raw = process.env.DBDOG_CASE_FEED_URL?.trim() || process.env.CLAUDE_PLUGIN_OPTION_CASE_FEED_URL?.trim() || "";
  return raw.replace(/\/+$/, "");
}
