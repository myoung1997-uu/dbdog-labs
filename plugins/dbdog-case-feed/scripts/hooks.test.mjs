...
test("平台地址：环境变量优先，且去掉尾部斜杠", () => {
  withEnv({ DBDOG_CASE_FEED_URL: "http://a:1/", CLAUDE_PLUGIN_OPTION_PLATFORM_URL: "http://b:2" }, () => {
    assert.equal(platformUrl(), "http://a:1");
  });
});

test("平台地址：环境变量没给时退回插件配置里的值", () => {
  withEnv({ DBDOG_CASE_FEED_URL: undefined, CLAUDE_PLUGIN_OPTION_PLATFORM_URL: "http://b:2/" }, () => {
    assert.equal(platformUrl(), "http://b:2");
  });
});

test("平台地址：都没有时是空串 —— 绝不能偷偷有个默认值", () => {
  withEnv({ DBDOG_CASE_FEED_URL: undefined, CLAUDE_PLUGIN_OPTION_PLATFORM_URL: undefined }, () => {
