---
name: judge-run
description: 判一轮诊断——从诊断表领走「待判题」的复现（一次复现一行），一例一个会话按同插件 diag-judge 的口径判：结论对不对、证据撑不撑得住、问题一条一条只分两类——确定是 bug（交判定链，下游自动修）/ 要人定（交全部上下文）；开判前把诊断表那行推到「判题中」，判完回流批注再推到「已判」。在线是默认：有答案纸就从根因倒推该有哪些证据，逐条去活系统取到手。触发词：judge-run / 判题 / 判一轮 / 判诊断 / 判这批诊断。
---
...
产出两样：**这次判成什么**，以及 **挖出的问题**（确定是 bug 的交给第四棒 `fix-run` 自动修，要人定的等人拍板）。后者才是这条 loop 的 KPI——
...
跑完告诉用户：判了几例、结论分布、**挖出几条问题（确定是 bug 几条、要人定几条）**。最后一项是这条 loop 存在的理由，
别把它埋在日志里。
