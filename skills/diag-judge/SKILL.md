...
修没修好由后续轮次说了算，不由人标。开判第一件事拿到这道题还没关的清单（`open-findings.json` 或 `case-history.mjs --open`），
...
关不关由脚本按后续复验算（true bug 一轮 `fixed` 即关；说法会误导要连续两轮），你只交 checks。
...
修复 loop（`fix-run`）改完打 `claimed_fixed` 标记、部署，页面上这道题显「已修待复现」；
人点「重复现」，下一轮 `diag-run` / `judge-run` 跑到那条路、你复验出 `fixed` 才算关。
...
- 「X 修好了没有」→ 不由你也不由修的人说了算：重跑挖出它的那道题、判那一轮，看复验是不是 `fixed`。
